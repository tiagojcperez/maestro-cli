...
├── codebase_map.py    # Interop reader for context_mode: codebase_map (Understand-Anything knowledge graph)
├── scip.py            # Interop reader for context_mode: scip (SCIP code-intelligence index, JSON form; multi-language)
...
| `codebase_map.py` | Interop reader for `context_mode: codebase_map` (Understand-Anything graph) | `build_codebase_map_context()`, `load_codebase_graph()`, `parse_codebase_graph()`, `format_codebase_map()`, `CodebaseGraph` |
| `scip.py` | Interop reader for `context_mode: scip` (SCIP index, JSON form; multi-language) | `build_scip_context()`, `load_scip_index()`, `parse_scip_index()`, `format_scip_map()`, `ScipIndex`, `ScipSymbol` |
...
- Error codes: E001-E072 (validation), E100-E110 (runtime); displayed as `[E001] message`
  ...
  - E073: `on_grant_violation` set without `allowed_tools`, or invalid value (must be `warn` or `fail`)
...
- Policy engine: `policy.py` implements safe AST-based expression evaluation; `compile_policy()` parses rules via `ast.parse(mode="eval")` and builds `_SafeEvaluator`; NEVER uses eval()/exec(); whitelisted task fields (`id`, `engine`, `model`, `tags`, `timeout_sec`, `max_retries`, `allow_failure`, `requires_approval`, `cache`, `description`, `allowed_tools`, `has_allowed_tools`, `has_scoped_tools`) + computed (`cost_usd`, `has_judge`, `execution_profile`); whitelisted plan fields (`name`, `max_cost_usd`, `max_parallel`, `execution_profile`, `fail_fast`); operators: `==`, `!=`, `<`, `>`, `<=`, `>=`, `and`, `or`, `not`, `in`, `not in`
...
- `allowed_tools`: per-task list of tool names the engine agent is allowed to use; also settable via `defaults.<engine>.allowed_tools`; for Claude, maps to `--disallowedTools` (inverse); for Codex, maps to sandbox levels; for other engines (gemini, copilot, qwen), enforced via system prompt injection; for ollama/llama, advisory only (W27); `CLAUDE_TOOLS` constant defines known Claude tools (`Read`, `Write`, `Edit`, `Bash`, `Glob`, `Grep`, `WebSearch`, `WebFetch`, `TodoWrite`); `TOOL_CATEGORIES` provides shorthand categories: `read-only` (no write/edit/bash), `no-shell` (everything except Bash); categories expand to per-engine tool lists via `_expand_tool_categories()`; E071 if set on command/group tasks; `has_allowed_tools` and `has_scoped_tools` available in policy engine rules
- Parameter-scoped tool grants (v2.5.4): `allowed_tools` entries accept `Tool(pattern)` specifiers (e.g. `Bash(git *)`, `Write(src/*)`) granting a tool only for arguments matching the glob pattern; for Claude the specifiers are passed natively via `--allowedTools` (auto-approve matching calls; non-matching calls are denied in headless `--print` mode unless a permission-bypass flag is present); all engines with observable tool-call streams get post-hoc verification — `check_tool_grants()` in runners.py matches each observed call's primary argument (`Bash`→command, `Read`/`Write`/`Edit`→file_path, `WebFetch`→url, etc.; JSON-serialized input as fallback) against the grants via `fnmatch`, with backslash→forward-slash normalization for Windows paths; a bare grant (`Bash`) wins over a scoped grant for the same tool; violations land in `TaskResult.grant_violations` and emit the `tool_grant_violation` event; `on_grant_violation: warn|fail` (task-level, default `warn`, requires `allowed_tools`, E073) — `fail` marks an otherwise-successful task failed post-hoc (like trajectory_guard `abort`, no retry); part of the task cache key
