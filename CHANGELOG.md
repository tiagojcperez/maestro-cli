...
- **Tested-guarantees doc + security-audit demo** — new [docs/TESTED_GUARANTEES.md](docs/TESTED_GUARANTEES.md) states honestly which engines have real end-to-end tests (codex/ollama) versus unit+integration only, and what runs in default CI. A new `demo-audit.tape` + `examples/risky_plan.yaml` record a `maestro audit` GIF showing a structurally valid plan flagged as unsafe.
