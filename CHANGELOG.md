...
- **Redundant coverage backends** — CI now uploads the same coverage to Coveralls (tokenless, badge in README) alongside Codecov, plus gated-on-secret integrations for SonarCloud (`sonarcloud.yml`), Codacy, and Code Climate. Every uploader is non-blocking, so CI stays green until each service is enabled. Setup steps + badge snippets in [docs/COVERAGE_PLATFORMS.md](docs/COVERAGE_PLATFORMS.md).
