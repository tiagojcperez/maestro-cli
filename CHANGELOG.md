...
- **`MAESTRO_FTS_PREFIX` — opt-in prefix matching** — set `MAESTRO_FTS_PREFIX=1` so FTS5 query tokens match longer tokens sharing their prefix (`auth` → `authentication`), trading precision for recall in knowledge + selective retrieval. Off by default; tokens shorter than 3 chars stay exact so a one-letter prefix can't match nearly everything. `rank_documents`/`relevance_by_rank` gained a `prefix=` parameter.
